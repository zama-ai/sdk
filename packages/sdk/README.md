...
| `delegateDecryption(delegate, options?)`   | Grant decryption rights to another address via the on-chain ACL. Options: `{ expirationDate: Date }` (default: permanent). ACL address resolved from relayer config.                                         |
| `revokeDelegation(delegate)`               | Revoke decryption delegation for this token. ACL address resolved from relayer config.                                                                                                                       |
...
| `decryptBalanceAs(delegator, opts?)`  | Decrypt a delegator's balance as a delegate. Options: `{ owner: Address }`. ACL address resolved from relayer config. |
