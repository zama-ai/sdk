...
- **Storage and permitStorage need distinct stores.** If you override the SDK's default `storage` (transport key pair persistence) and `permitStorage` (permit cache), give them distinct backing stores. Sharing one store causes the permit entry to overwrite the encrypted transport key pair, forcing a fresh EIP-712 sign on every decrypt. Omitting both is safe — the defaults are already separated.
