...
test("should show metadata for confidential wrapper token (cUSDT)", async ({ page, contracts }) => {
  ...
  await expect(page.getByTestId("metadata-is-wrapper")).toHaveText("true");
});

test("should show metadata for confidential wrapper token (cERC20)", async ({
  page,
  contracts,
}) => {
