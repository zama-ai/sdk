...
- **Token-specific hooks include the domain.** `useConfidentialBalance`, `useConfidentialTransfer`, `useShield`, `useUnshield`. These are explicitly token-flavored operations.
- **Subpath hooks omit the subpath's domain.** Under `@zama-fhe/react-sdk/vaults` it is `useDeposit` / `useJoin` / `useClaim`, not `useVaultDeposit`: the import path is the namespace (the `/wagmi` entry already exports its own `createConfig` on the same principle), hook names mirror the SDK method they wrap (`useDeposit` ↔ `Vault.deposit`), and the `useVault*` names are taken by the hooks that return the object itself. Subpath hooks are never re-exported from the root entry.
