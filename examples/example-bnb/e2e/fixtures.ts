...
// On-chain WrappersRegistry address for BNB (DefaultRegistryAddresses).
export const REGISTRY_ADDRESS = "0xc0E8B73b1C58D846e1d4f8fAE2E1466C85BCeAeC";
...
export interface RpcOptions {
   ...
   *
   * Note: in the bnb app, eth_call is NOT routed through window.ethereum — the hybrid
   * provider sends all contract reads directly to the BNB HTTP RPC. The emptyRegistry
 ...
 * - `eth_call` is NOT handled here — the app's JsonRpcProvider (providers.tsx) routes
 *   all contract reads (eth_call, eth_estimateGas) directly to the BNB HTTP RPC,
...
async function injectMockWallet(page: Page, config: WalletConfig) {
  await page.addInitScript((cfg: WalletConfig) => {
    ...
    // with a fresh ethers adapter bound to the new account.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
/**
 * Intercepts HTTP requests to the BNB RPC endpoint and returns minimal
 * valid JSON-RPC responses.
 *
 * In the bnb app, the hybrid EIP-1193 provider routes all eth_call and
 * eth_estimateGas requests directly to the BNB HTTP RPC — bypassing
...
async function interceptRpc(page: Page, options: RpcOptions = {}) {
  await page.route("**/bsc-testnet-rpc.publicnode.com**", async (route) => {
    ...
    /** BNB WrappersRegistry — lowercased for comparison with request `to` fields. */
    const REGISTRY = REGISTRY_ADDRESS.toLowerCase();

    /** BNB testnet ACL contract (see sdk chains/configs.ts) — used to mock delegation-status reads. */
    const ACL = "0x52470e945521e247cb4754088a836dc4b838afbe";
...
interface TestFixtures {
  ...
  /** Call to intercept BNB RPC requests with static responses. */
