...
describe("clear signing intent builders", () => {
  ...
  test("buildAllowAsIntentFromEIP712 maps delegated decrypt typed data", () => {
    ...
    expect(intent.warnings).toContain(
  ...
  test("builders omit optional undefined properties", () => {
    ...
    expect(shieldIntent.rawContext).toEqual({ route: "approveAndWrap" });
    expect(shieldIntent.rawContext).not.toHaveProperty("contractCalls");
  });
});
