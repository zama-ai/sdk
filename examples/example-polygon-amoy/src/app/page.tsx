...
export default function Home() {
  ...
  // comes from the chain config's registryAddress in src/providers.tsx
  // (Polygon Amoy: 0xF486c3D4F4562760A43883e72E8D6f6Cf2EFdA94), so nothing to pass here.
  ...
  // Native POL balance via wagmi's useBalance, routed through the configured Polygon Amoy
  // transport, so it stays independent of the injected wallet's own RPC endpoint.
  const { data: nativeBalance } = useBalance({
    address,
    query: { enabled: isConnected && isAmoy },
  });
  ...
  if (!isConnected || !address) {
    ...
    return (
      <ActionScreen
        ...
        description="Connect your wallet to interact with ERC-7984 tokens on Polygon Amoy testnet."
  ...
  if (!isAmoy) {
    return (
      <ActionScreen
        title="Polygon Amoy Network Required"
        description={`This app only works on Polygon Amoy testnet (chain ID ${AMOY_CHAIN_ID}).`}
        actionLabel="Switch to Polygon Amoy"
        pendingLabel="Switching…"
        pending={isSwitching}
        onAction={() => switchChain({ chainId: polygonAmoy.id })}
        error={
          switchError && "Could not switch to Polygon Amoy. Please switch manually in your wallet."
        }
      />
    );
  }

  // ── Screen 3: Connected on Polygon Amoy, main UI ─────────────────────────────────
  return (
