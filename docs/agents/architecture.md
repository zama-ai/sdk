...
**Transparent routing:** the SDK routes between host-chain RPC and Relayer API automatically. Callers never choose which backend to use. Relayers are selected by factory — `web()` (browser), `node()` (server), `cleartext()` (cleartext chains) — not instantiated directly; each produces a runtime implementing the same `RelayerSDK` interface, and `ChainRouter` routes per chain, so SDK and Token code never branches on the runtime.
