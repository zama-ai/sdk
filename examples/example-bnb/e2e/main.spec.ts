...
// All tests in this suite start with the wallet already connected on BNB.
...
test.describe("main screen", () => {
  ...
  test("shows token selector populated from the registry", async ({ page }) => {
    ...
    // (the app routes eth_call (via its JsonRpcProvider) directly to the BNB HTTP RPC).
  ...
  test("shows dash for ERC-20 balance before data loads", async ({ page }) => {
    ...
    // button is shown instead of a balance value, so .balance-value elements are ERC-20 only.
  ...
  test("tBNB balance shows in header", async ({ page }) => {
    // tBNB row is always rendered; shows "—" until the query resolves.
    await expect(page.getByText(/tBNB:/)).toBeVisible();
  });
  ...
  test("mint button is enabled once the registry loads a token", async ({ page }) => {
    // actionsDisabled = !isBnb || !token. Once the first pair is auto-selected,
...
// emptyRegistry is passed to mockRpc (not mockWallet) because the app routes
// eth_call directly to the BNB HTTP RPC (via its JsonRpcProvider), bypassing window.ethereum.
test.describe("registry empty state", () => {
  ...
  test("action controls are absent when no tokens are available", async ({ page }) => {
    // With no registered pairs the per-token action panel is not rendered at all —
    // Shield/Transfer/Unshield/Mint and the Balances card all live inside it.
    await expect(page.getByText("No tokens available.")).toBeVisible();
    await expect(page.getByRole("button", { name: "Shield", exact: true })).not.toBeVisible();
    await expect(page.getByRole("button", { name: "Transfer", exact: true })).not.toBeVisible();
    await expect(page.getByRole("button", { name: "Unshield", exact: true })).not.toBeVisible();
