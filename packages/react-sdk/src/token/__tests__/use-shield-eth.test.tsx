...
describe("useShieldETH", () => {
  ...
  test("cache: invalidates allowance and removes balance after shield ETH", async ({
    renderWithProviders,
    signer,
  }) => {
  ...
  test("behavior: forwards onSuccess callback", async ({ renderWithProviders, signer }) => {
    ...
    const { result, queryClient } = renderWithProviders(() =>
      useShieldETH({ tokenAddress: TOKEN, wrapperAddress: WRAPPER }, { onSuccess }),
    );
    ...
    await mutateAndExpectOnSuccess(
      () => result.current.mutateAsync({ amount: 1000000000000000000n }),
      onSuccess,
      (client) => {
