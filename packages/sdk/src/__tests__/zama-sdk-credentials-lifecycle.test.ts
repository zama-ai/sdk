...
describe("ZamaSDK credentials lifecycle", () => {
  test("re-signs after permitTTL elapses but reuses the FHE keypair", async ({
    ...
    await sdk.permits.allow([CONTRACT_A]);
  ...
  describe("chain-switch isolation", () => {
    test("isAllowed on a different chain returns false and allow re-signs", async ({
      ...
      await sdkA.permits.allow([CONTRACT_A]);
      expect(signerA.signTypedData).toHaveBeenCalledOnce();
      expect(await sdkA.permits.isAllowed([CONTRACT_A])).toBe(true);
      ...
      expect(await sdkB.permits.isAllowed([CONTRACT_A])).toBe(false);

      await sdkB.permits.allow([CONTRACT_A]);
      ...
      expect(await sdkA2.permits.isAllowed([CONTRACT_A])).toBe(true);
      await sdkA2.permits.allow([CONTRACT_A]);
  ...
  describe("reload round-trip", () => {
    test("a fresh SDK with the same storage reuses the persisted permit", async ({
      ...
      await sdkA.permits.allow([CONTRACT_A, CONTRACT_B]);
      expect(signerA.signTypedData).toHaveBeenCalledOnce();
      expect(await sdkA.permits.isAllowed([CONTRACT_A, CONTRACT_B])).toBe(true);
      ...
      expect(await sdkB.permits.isAllowed([CONTRACT_A, CONTRACT_B])).toBe(true);

      await sdkB.permits.allow([CONTRACT_A, CONTRACT_B]);
