 ...
 *
 * Non-React primitives (contract builders, event decoders, error classes,
 * storage implementations, network configs, query-layer utilities, etc.)
 * are not re-exported from this entrypoint. Import them directly from
 * `@zama-fhe/sdk` (or `@zama-fhe/sdk/query` for the query-layer helpers) —
 * both are available as transitive dependencies of `@zama-fhe/react-sdk`.
 *
