...
describe("DelegationStore", () => {
  it("is not active before any event is applied", () => {
    const store = new DelegationStore();
    expect(store.isKnownActive(DELEGATOR, CONTRACT)).toBe(false);
  });

  it("becomes active after a grant", () => {
    const store = new DelegationStore();
    store.apply([record({ action: "granted", blockNumber: 100n })]);
    expect(store.isKnownActive(DELEGATOR, CONTRACT)).toBe(true);
    expect(store.list()).toHaveLength(1);
  });

  it("becomes inactive after a later revoke", () => {
    const store = new DelegationStore();
    store.apply([
  ...
  it("ignores an out-of-order (stale) event applied after a newer one", () => {
    const store = new DelegationStore();
    store.apply([record({ action: "revoked", blockNumber: 200n })]);
    // A grant from an earlier block arriving late (e.g. re-org, or unordered fetch) must not resurrect it.
    store.apply([record({ action: "granted", blockNumber: 100n })]);
    expect(store.isKnownActive(DELEGATOR, CONTRACT)).toBe(false);
  });

  it("re-grant after revoke (same block ordering) re-activates", () => {
    const store = new DelegationStore();
    store.apply([
  ...
  it("tracks separate (delegator, contractAddress) pairs independently", () => {
    const store = new DelegationStore();
    const otherContract = "0x0000000000000000000000000000000000dEaD" as const;
    store.apply([record({ contractAddress: otherContract, action: "granted" })]);
    expect(store.isKnownActive(DELEGATOR, otherContract)).toBe(true);
    expect(store.isKnownActive(DELEGATOR, CONTRACT)).toBe(false);
  });
});
