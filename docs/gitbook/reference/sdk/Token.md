...
await token.shield(1000n); // [!code focus]
...
// Another address
const balance = await token.balanceOf("0xOwnerAddress"); // [!code focus]
```
...
```ts
await token.confidentialTransfer("0xRecipient", 500n); // [!code focus]
```
...
```ts
await token.confidentialTransferFrom("0xFrom", "0xTo", 500n); // [!code focus]
```
...
```ts
await token.unshield(500n); // [!code focus]

// With progress callbacks
await token.unshield(500n, {
  onUnwrapSubmitted: (txHash) => updateUI("Unwrap submitted..."), // [!code focus]
  onFinalizing: () => updateUI("Waiting for decryption proof..."), // [!code focus]
  onFinalizeSubmitted: (txHash) => updateUI("Done!"), // [!code focus]
});
```
...
```ts
await token.unshieldAll(); // [!code focus]
```
...
if (pending) {
  await token.resumeUnshield(pending); // [!code focus]
...
await token.approve("0xSpender"); // [!code focus]

// Approve until a specific timestamp
await token.approve("0xSpender", futureTimestamp); // [!code focus]
```
...
```ts
const approved = await token.isApproved("0xSpender"); // [!code focus]
```
...
```ts
const allowed = await token.isAllowed(); // [!code focus]
```
