...
- **Provider.** `GenericProvider` gains `prepareTransaction({ from, calldata })`, which resolves chain ID, nonce (`pending` block tag), gas limit, and EIP-1559 fees from chain state and returns the RLP-encoded unsigned tx. Optional `nonce`/`gasLimit`/`maxFeePerGas`/`maxPriorityFeePerGas` overrides let a custodian pin its own nonce and fee values (`maxFeePerGas` and `maxPriorityFeePerGas` must be pinned together or both omitted). Wired in the viem, ethers, and wagmi adapters.
