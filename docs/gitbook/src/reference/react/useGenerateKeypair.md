...
{% hint style="warning" %}
[`useUserDecrypt`](/reference/react/useUserDecrypt), [`useGrantPermit`](/reference/react/useGrantPermit), and [`useConfidentialBalance`](/reference/react/useConfidentialBalance) handle keypair generation automatically. Call `useGenerateKeypair` only when managing FHE keypairs manually.
{% endhint %}
...
function GenerateButton() {
  const { mutateAsync: generateKeypair, isPending, data: keypair } = useGenerateKeypair();
...
- [`useGrantPermit`](/reference/react/useGrantPermit) — high-level hook that generates a keypair and signs permits
