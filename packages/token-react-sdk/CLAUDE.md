...
`@zama-fhe/token-react-sdk` — React hooks for Zama confidential ERC-20 tokens (fhEVM). Built on `@tanstack/react-query`, wrapping `@zama-fhe/sdk` (workspace dependency). Part of a pnpm monorepo (`@zama-fhe/sdk-monorepo`).
