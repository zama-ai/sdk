...
{% hint style="warning" %}
This self-healing behavior only applies when `transportKeyPairScope` is **not** set. Combined with a shared scope, every instance sharing that scope must use the _same_ `derivationSecret` at all times. An instance with a stale or inconsistent secret doesn't get its own extra re-prompt — reading a scope's shared entry with the wrong secret throws instead of silently regenerating, specifically so a misconfigured instance can never overwrite (and thereby invalidate, for every signer in the scope) a peer's correctly-wrapped entry. Roll `derivationSecret` out to every instance sharing a scope atomically, or expect this error during the transition window.
{% endhint %}
