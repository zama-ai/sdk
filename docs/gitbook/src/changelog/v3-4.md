...
{% hint style="warning" %}
**Sharing only works if every signer in the scope reads and writes the same storage instance.** The Node `asyncLocalStorage` default isolates a fresh store per request, which would silently regenerate and discard the "shared" key pair on every request.
{% endhint %}
...
The result shape is exported as the `DelegationStatus` type from `@zama-fhe/sdk`. In React, the existing [`useDelegationStatus`](../reference/react/useDelegationStatus.md) hook already returns this data and needs no changes.
...
See [Check delegation status](../guides/delegated-decryption.md#4-check-delegation-status-optional) for the full flow.
