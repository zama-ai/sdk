...
`Vault` (`createVault`, or `useVault` in React) pairs a vault's deposit and redeem batchers behind ERC-4626-style `deposit` and `redeem` methods. Both encrypt the amount, check the caller's confidential balance the way `Token.confidentialTransfer` does, grant the batcher an operator approval when one isn't already active, and return the `batchId` they joined. The React hooks are `useDeposit` and `useRedeem`.
