...
Start with `llms.txt` for normal coding tasks and let the agent follow the raw GitHub links to the smallest relevant source; reach for `llms-full.txt` only when you want everything loaded at once. The `source_path` values such as `docs/gitbook/src/...` are provenance metadata, not local paths — if you haven't cloned the repo, use the raw GitHub URLs (working inside a clone, read those paths directly).
