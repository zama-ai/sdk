# Developer Walkthrough — react-wagmi clear signing

A step-by-step guide to how this app integrates app-level clear-signing intent previews with
`@zama-fhe/react-sdk` using wagmi v3.
...
├── ClearSigningConsole.tsx      — human-readable intent preview + raw JSON display
...
const allowContracts = validPairs.map((p) => p.confidentialTokenAddress);
const allowPreview = useAllowClearSigningIntent();
const allowTokens = useMutation({
  mutationFn: () =>
    sdk.allow(allowContracts, {
...
`useConfidentialBalance` is gated by `isAllowed`, so the first decrypt only happens after
the explicit "Decrypt Balance" click. `Preview intent` generates the same human-readable
clear-signing intent without opening the wallet.

---

## 7. Operation previews

Each operation card keeps the normal SDK execution flow and adds a sibling preview mutation:
...
- Decrypt authorization: `useAllowClearSigningIntent` + `sdk.allow`
- Shield: `useShieldClearSigningIntent` + `useShield`
- Confidential transfer: `useConfidentialTransferClearSigningIntent` + `useConfidentialTransfer`
- Unshield: `useUnwrapClearSigningIntent` + `useUnshield`
- Delegate decryption: `useDelegateDecryptionClearSigningIntent` + `useDelegateDecryption`

`Preview intent` updates `ClearSigningConsole` without submitting a signature or transaction.
The execute button still passes `onClearSigningIntent`, so the console also captures runtime
intents immediately before the wallet prompt.
...
pnpm --filter react-wagmi-clear-signing-example build
pnpm --filter react-wagmi-clear-signing-example typecheck
pnpm --filter react-wagmi-clear-signing-example test:e2e
```
