...
deploy_contracts() {
  ...
  # forge-fhevm host stack — the CleartextFHEVMExecutor.plaintexts model plus the
  # mock coprocessor/KMS signer keys the off-chain CleartextRelayer signs with.
  # Its `hardhat` preset addresses + gateway id match deploy-local.sh's defaults,
  # so the proofs the relayer produces verify on-chain. Independent per port
  # (setCode + cast-send --create, no forge broadcast cache), so it runs outside
  # the lock. Skip the internal forge build in CI, where artifacts are prebuilt
  # and soldeer deps may be absent.
  local deploy_args=(--anvil-port "$PORT")
  [ -n "${CI:-}" ] && deploy_args+=(--skip-build)
  if ! "$FORGE_FHEVM_DIR/deploy-local.sh" "${deploy_args[@]}"; then
    echo "forge-fhevm host stack deploy failed on port $PORT" >&2
    return 1
  fi

  acquire_lock || return 1

  # Clear broadcast cache — both anvils share chain-id 31337, so a second run
