...
{% hint style="warning" %}
The receiver hook only applies to deployed contracts. If `to` is a deployed contract that doesn't implement the hook (or the hook reverts), the whole transaction reverts with `TransactionRevertedError`. If `to` has no code — a plain wallet address, or a contract that hasn't been deployed yet — the hook is silently skipped and the transfer succeeds like a plain transfer, with no way for the recipient to react. Use plain [`confidentialTransfer`](../guides/transfer-privately.md) for ordinary recipients.
{% endhint %}
