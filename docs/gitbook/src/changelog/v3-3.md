...
`runtime.auth` is a process-wide fallback; a per-chain `auth` set on the chain preset takes precedence for that chain. Because the underlying runtime is one-shot, conflicting `runtime` configs across chains throw at setup.
