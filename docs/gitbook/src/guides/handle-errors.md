...
| `RevokedKmsContextError`               | The SDK already evicted the dead permit. With a `SigningFailedError` as `cause`, the re-grant failed and the scope's other permits were kept: establish a new permit. Otherwise retry after ~15 minutes (the on-chain validity check is cached). |
