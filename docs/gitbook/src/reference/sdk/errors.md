...
> **Cost:** the entitlement pre-check issues one `persistAllowed` read per uncached handle on the consumer's RPC provider, in addition to the relayer's own internal ACL check. This is the deliberate trade-off for a typed, version-independent error (vs. fragile message-matching); factor it into your RPC rate budget.
