...
Sometimes the recipient is a contract that needs to _react_ to the transfer — for example a confidential vault that credits a deposit, or a payment splitter that fans the amount out. `confidentialTransferAndCall` moves the encrypted amount **and** invokes the recipient's ERC-7984 receiver hook (`onConfidentialTransferReceived`) in a single transaction, so the deposit can never land without the contract being told about it. If the recipient doesn't implement the hook, the call reverts with `TransactionRevertedError` — transfer and hook invocation are atomic, so there's no way to land only the transfer.
