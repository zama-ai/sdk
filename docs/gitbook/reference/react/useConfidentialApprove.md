...
```tsx [ApproveOperator.tsx]
import { useConfidentialApprove } from "@zama-fhe/react-sdk";

function ApproveOperator({ tokenAddress }: { tokenAddress: Address }) {
  const { mutateAsync: approve, isPending } = useConfidentialApprove({ tokenAddress }); // [!code focus]

  const handleApprove = async () => {
    const txHash = await approve({ spender: "0xDEX" }); // [!code focus]
...
- [`useConfidentialTransferFrom`](/reference/react/query-keys#useconfidentialtransferfrom) — operator transfer using an existing approval
