...
- **Shielding (public ERC-20 → confidential).** Use `Token.shield(amount, options?)` (or `useShield`). The SDK detects ERC-1363 support on the underlying ERC-20 and routes through `transferAndCall` (single tx) or `approve` + `wrap` (two txs) automatically — callers never pick the path. Don't compose `erc20.approve` + `wrapper.wrap` (or `erc20.transferAndCall`) by hand, don't branch on token address, and don't expose a per-call flag for it. See [`docs/gitbook/src/guides/shield-tokens.md`](docs/gitbook/src/guides/shield-tokens.md) for the routing table.
- **Unshielding (confidential → public ERC-20).** Use `Token.unshield` / `Token.unshieldAll` / `useUnshield` — they orchestrate the two-phase request-then-finalize flow. The lower-level `unwrap/*` hooks are escape hatches for callers that genuinely need per-phase control.
