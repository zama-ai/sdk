...
describe("hex prefix helpers", () => {
  test("adds a prefix when the relayer SDK returns bare hex", () => {
    expect(prefixHex("abcd")).toBe("0xabcd");
  });

  test("preserves an existing 0x prefix", () => {
    expect(prefixHex("0xabcd")).toBe("0xabcd");
  });

  test("removes the 0x prefix for relayer SDK inputs", () => {
    expect(unprefixHex("0xabcd")).toBe("abcd");
  });

  test("throws when value lacks 0x prefix", () => {
