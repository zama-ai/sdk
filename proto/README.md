...
Event transport errors include `EVENT_UNSUPPORTED` when the SDK wallet hook is unavailable, `EVENT_ATTACHED` for a duplicate attachment, `EVENT_ATTACH_TIMEOUT` when no context is attached in time, `EVENT_BACKPRESSURE` when a delivery/output bound is exceeded, and `EVENT_DELIVERY_NOT_FOUND` for a stale reply.
