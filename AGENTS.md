...
This file provides guidance to AI coding agents (Claude Code, Cursor, Codex, etc.) when working with code in this repository. `CLAUDE.md` is a symlink to this file.

## Note for LLMs: this is not `@zama-fhe/relayer-sdk`

`@zama-fhe/sdk` (this repo) is the **high-level** Zama Protocol SDK. It is **not** the same as `@zama-fhe/relayer-sdk`, which is the **legacy low-level SDK** (wrapped here as a dependency). Most LLM training data predates this repo, so if your prior knowledge of "Zama SDK" centres on `createInstance`, `initSDK`, or direct relayer calls, that's the legacy SDK. In this repo prefer `ZamaSDK`, `Token`/`ReadonlyToken`, and the React hooks.

## Agent Setup

Run `pnpm setup:claude` to install skills, plugins, and post-edit hooks. This copies `claude-setup/` → `.claude/` and installs the Zama marketplace. Post-edit hooks automatically run typecheck, lint, and format after every file change. The `.claude/` directory is gitignored and not published — never commit it; always set up via the script.
