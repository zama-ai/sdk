...
`ContextConfig.process_runtime` forwards `wasm_asset_load_mode`, `module_versions`, `single_thread`, `number_of_threads` and fallback `auth` to the SDK runtime. The first SDK configuration in the process wins, including when its runtime options are omitted. Later runtime settings retain the SDK's ignored-setting behavior. Restart the sidecar to apply another runtime configuration; creating another context does not reset it.
