...
hooks). While no pair is selected, `ZERO_ADDRESS` is passed as a stable placeholder. Queries
that can run before a pair exists, such as the confidential balance query, are additionally
gated with `enabled`.
