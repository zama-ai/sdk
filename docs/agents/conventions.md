...
- **Key/keypair terms mirror the glossary too** (`TransportKeyPair` / `generateTransportKeyPair()`, "FHE encryption key" / `fetchFheEncryptionKeyBytes()` — not `keypair` / `getPublicKey`), and renames are a **clean break with no back-compat aliases** (pre-GA on 3.x; cf. #386). Canonical source: the FHEVM `devex/js-sdk` GLOSSARY (= DevEx Notion); `main` lags.
