...
await token.approve("0xSpender", expiry); // [!code focus]
```
...
function ApprovalPanel({ tokenAddress }: { tokenAddress: string }) {
  const { mutateAsync: approve, isPending: isApproving } = useConfidentialApprove({ tokenAddress }); // [!code focus]

  const { data: isApproved } = useConfidentialIsApproved({ tokenAddress, spender: "0xSpender" }); // [!code focus]
