...
> **Separate IndexedDB instances** for `storage` and `permitStorage` in `ZamaProvider`: both use the same key internally, so sharing a single `IndexedDBStorage` instance causes the permit entry to overwrite the encrypted transport key pair, forcing a re-sign on every balance decryption.
