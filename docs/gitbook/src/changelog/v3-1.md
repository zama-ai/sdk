...
Each transport factory also takes an optional `options` argument for per-chain tuning — in `3.1.0` this parameter was renamed from `relayer` to `options`. Its fields depend on the FHE backend, which changed in `3.3.1`; see [FHE runtime and client tuning](v3-3.md#fhe-runtime-and-client-tuning) for the current options.
