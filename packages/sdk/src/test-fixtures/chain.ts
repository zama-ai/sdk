 ...
 * that schema-validate the chain shape (e.g. `RelayerRouter`).
 *
 * `protocolConfigContractAddress` defaults to set (like mainnet/sepolia/polygon),
 * so V2/unified-permit code paths are reachable by default; pass `undefined`
 * explicitly to simulate a chain without `ProtocolConfig` deployed (e.g. the
 * real `hardhat`/`anvil` preset).
 */
function createMockChain(overrides: Partial<FheChain> & { id: number }): FheChain {
  return {
    ...
    protocolConfigContractAddress: STUB_ADDRESS,
