...
try {
  ...
  if (error instanceof SigningRejectedError) {
    // [!code focus]
    // User clicked "Reject" in their wallet           // [!code focus]
  } else if (error instanceof EncryptionFailedError) {
    // [!code focus]
    // FHE encryption failed                            // [!code focus]
  } else if (error instanceof ZamaError) {
    // [!code focus]
    // Some other SDK error -- check error.code         // [!code focus]
  } else {
    // Not an SDK error
  }
}
...
matchZamaError(error, {
  // [!code focus]
  SIGNING_REJECTED: () => toast("Please approve the transaction"), // [!code focus]
  ENCRYPTION_FAILED: () => toast("Encryption failed -- please retry"), // [!code focus]
  TRANSACTION_REVERTED: (e) => toast(`Transaction failed: ${e.message}`), // [!code focus]
  _: () => toast("Something went wrong"), // [!code focus]
}); // [!code focus]
```
...
try {
  ...
  showBalance(balance); // [!code focus]
} catch (error) {
...
function ErrorMessage({ error }: { error: Error | null }) {
  ...
  const message = matchZamaError(error, {
    ...
    SIGNING_REJECTED: () =>
      // [!code focus]
      "Transaction cancelled -- please approve in your wallet.", // [!code focus]
    ENCRYPTION_FAILED: () =>
      // [!code focus]
      "Encryption failed -- please try again.", // [!code focus]
    TRANSACTION_REVERTED: () =>
      // [!code focus]
      "Transaction failed on-chain -- check your balance.", // [!code focus]
    _: () => "Something went wrong.", // [!code focus]
  }); // [!code focus]
...
| `EncryptionFailedError`                   | Web Worker cannot load WASM                 | Check your CSP headers -- the worker needs `wasm-unsafe-eval`.                                      |
