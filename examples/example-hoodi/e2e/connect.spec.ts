import { test, expect, HOODI_CHAIN_ID_HEX, TEST_ADDRESS } from "./fixtures";
...
test.describe("connect flow", () => {
  ...
  test("shows connect screen title", async ({ page }) => {
    await page.goto("/");
    await expect(
      page.getByRole("heading", { name: "Hoodi Confidential Token Quickstart" }),
    ).toBeVisible();
  });
  ...
  test("auto-detects existing connection and shows main screen", async ({
    ...
    await mockRpc();
    // Wallet already connected on Hoodi — page auto-detects via useEffect.
