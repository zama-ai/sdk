...
const { txHash, receipt } = await token.unshield(500n); // [!code focus]
```
...
await token.unshield(500n, {
  onUnwrapSubmitted: (txHash) => {
    // [!code focus]
    updateUI("Unwrap submitted..."); // [!code focus]
  }, // [!code focus]
  onFinalizing: () => {
    // [!code focus]
    updateUI("Waiting for decryption proof..."); // [!code focus]
  }, // [!code focus]
  onFinalizeSubmitted: (txHash) => {
    // [!code focus]
    updateUI("Unshield complete!"); // [!code focus]
  }, // [!code focus]
});
```
...
await savePendingUnshield(storage, wrapperAddress, unwrapTxHash); // [!code focus]

// On next page load, check for pending unshields
const pending = await loadPendingUnshield(storage, wrapperAddress); // [!code focus]
if (pending) {
  await token.resumeUnshield(pending); // [!code focus]
  await clearPendingUnshield(storage, wrapperAddress); // [!code focus]
}
```
