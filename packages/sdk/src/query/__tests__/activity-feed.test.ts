...
describe("activityFeedQueryOptions", () => {
  ...
  test("enabled is false when userAddress is missing", () => {
    const token = createMockReadonlyToken(TOKEN);
    const options = activityFeedQueryOptions(token, { logs: [] });

    expect(options.enabled).toBe(false);
  });

  test("enabled is false when logs are missing", () => {
    const token = createMockReadonlyToken(TOKEN);
    const options = activityFeedQueryOptions(token, { userAddress: USER });
  ...
  test("queryFn skips decryption when decrypt is false", async () => {
    const token = createMockReadonlyToken(TOKEN);
