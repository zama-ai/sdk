...
The full Zama FHE stack relies on:
...
**BNB uses the cleartext stack.** In cleartext mode:

- Values are stored as **plaintexts** in the `CleartextFHEVMExecutor` contract on-chain (no actual FHE encryption)
- The `cleartext()` relayer transport acts as a local relayer: it reads plaintexts directly from the executor contract and produces mock KMS signatures locally
...
All on-chain operations require tBNB. Aim for at least **0.01 tBNB** before starting — shield and unshield each involve multiple transactions.

Recommended faucet:

- [BNB Chain testnet faucet](https://www.bnbchain.org/en/testnet-faucet) — official, drips tBNB
...
| --------- | -------------------------------------------- | -------------------------------------------- |
| USDC Mock | `0x1b3BC224c233D38Db8A92DA3fC44d01A9232b64c` | `0xbb9Ac1000B79a035B7Aa933cf6E44B51a2f6222a` |
| USDT Mock | `0xaA3E4C4db8D44711B6fc0E4ffdCBb3749C1A3A72` | `0x8278F319597d67bb72f6e2d3119F1970FBE86Dc8` |

Registry (DeploymentCoordinator): `0xc0E8B73b1C58D846e1d4f8fAE2E1466C85BCeAeC`

All contracts verified on [testnet.bscscan.com](https://testnet.bscscan.com).

> Token pairs are loaded dynamically from the on-chain WrappersRegistry (`0xc0E8B73b1C58D846e1d4f8fAE2E1466C85BCeAeC`) at runtime — `src/app/page.tsx` no longer contains a hardcoded `TOKENS` constant. If contracts are redeployed, the registry should be updated; the app will pick up the new addresses automatically.
...
1. Go to the ERC-20 contract on [testnet.bscscan.com](https://testnet.bscscan.com) (e.g., `0x1b3BC2...` for USDC Mock).
...
The app calls `eth_requestAccounts` to connect, then `wallet_switchEthereumChain` (or `wallet_addEthereumChain` if BNB is not yet known to the wallet) to switch to BNB (chainId 97 / `0x61`). No further setup is needed.

Once connected, your wallet address and tBNB balance appear at the top of the page.

If you switch to a different network after connecting, the app shows a full-page **BNB Network Required** screen with a **Switch to BNB** button. All operation buttons are disabled until you switch back.

### Step 2 — Select a token

Use the **Token** dropdown to select between available tokens. Token pairs are loaded from the on-chain **WrappersRegistry** via `useListPairs({ metadata: true })` — no hardcoded addresses needed. The registry address for BNB (`0xc0E8B73b1C58D846e1d4f8fAE2E1466C85BCeAeC`) is resolved automatically from the connected chain ID. Name, symbol, and decimal precision are included in the registry response.
...
Both phases complete within seconds on BNB. The ERC-20 balance refreshes automatically on success.
...
| Asked to sign an EIP-712 message after each action        | Permit not yet cached (first use — expected)                                                                                                             | Approve once — subsequent decryptions reuse the IndexedDB-persisted permit (30-day TTL). If the prompt recurs every time, make sure `storage` and `permitStorage` in `ZamaProvider` point to **different** `IndexedDBStorage` instances — sharing the same instance corrupts the stored credentials (see Providers setup)                                        |
| Shield fails immediately after approving spend cap        | Approval transaction not yet confirmed on BNB when wrap was attempted                                                                                    | Wait for the approval confirmation and retry — the app detects this and shows a hint                                                                                                                                                                                                                                                                             |
...
Covered flows: connect screen (no wallet, install error, page title, auto-detect, click-to-connect), wrong-network screen (display, chain ID, back-to-BNB transition), main UI (all cards rendered, connected address, tBNB balance, token selector, registry empty state, balance display, token switching, pending unshield absence, mint button state), delegation section (section labels, buttons disabled before address entry, Grant Access and Revoke Access enabled after valid address entry).

Tests run automatically on CI for every pull request that touches `examples/example-bnb/`.

---

## Going further

- **Additional tokens** — register new ERC-7984 pairs in the on-chain WrappersRegistry at `0xc0E8B73b1C58D846e1d4f8fAE2E1466C85BCeAeC`. The app picks them up automatically via `useListPairs` on the next load — no code change required.
- **Production use** — replace `cleartext()` with `web()` (browser) or `node()` (server) when targeting a chain with the full FHE co-processor.
