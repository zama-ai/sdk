# Table of contents

- [Overview](overview.md)

## Getting Started

- [Quick start](tutorials/quick-start.md)
- [First confidential dApp](tutorials/first-confidential-dapp.md)
- [Wallet & exchange integration](tutorials/wallet-exchange-integration.md)
- [Build with an LLM](tutorials/build-with-an-llm.md)
- [Supported environments](tutorials/supported-environments.md)

## Migration

- [Migrate from v2 to v3](guides/migrate-v2-to-v3.md)

## Changelog

- [Release channels](changelog/README.md)
- [Beta (unreleased)](changelog/beta.md)
- [3.x (current)](changelog/v3.md)
  - [3.6.x](changelog/v3-6.md)
  - [3.5.x](changelog/v3-5.md)
  - [3.4.x](changelog/v3-4.md)
  - [3.3.x](changelog/v3-3.md)
  - [3.2.x](changelog/v3-2.md)
  - [3.1.x](changelog/v3-1.md)
  - [3.0.x](changelog/v3-0.md)
- [Legacy versions](changelog/legacy.md)
  - [2.x](changelog/v2.md)
  - [1.x](changelog/v1.md)

## Guides

- [Configuration](guides/configuration.md)
- [Authentication](guides/authentication.md)
- [Relayer API keys](guides/relayer-api-keys.md)
- [Shield tokens](guides/shield-tokens.md)
- [Transfer privately](guides/transfer-privately.md)
- [Unshield tokens](guides/unshield-tokens.md)
- [Check balances](guides/check-balances.md)
- [Decrypt values from event logs](guides/decrypt-from-event-logs.md)
- [Handle errors](guides/handle-errors.md)
- [Node.js backend](guides/node-js-backend.md)
- [Offline signing](guides/offline.md)
- [Web extensions](guides/web-extensions.md)
- [Local development](guides/local-development.md)
- [Next.js SSR](guides/nextjs-ssr.md)
- [Operator approvals](guides/operator-approvals.md)
- [Delegated decryption](guides/delegated-decryption.md)
- [Encrypt & decrypt](guides/encrypt-decrypt.md)
- [Vault deposits and redemptions](guides/vault-deposits.md)
- [Vault groups](guides/vault-groups.md)

## API References

- [SDK reference](reference/sdk/README.md)
  - [ZamaSDK](reference/sdk/ZamaSDK.md)
  - [Token](reference/sdk/Token.md)
  - [WrappedToken](reference/sdk/WrappedToken.md)
  - [WrappersRegistry](reference/sdk/WrappersRegistry.md)
  - [Vault](reference/sdk/Vault.md)
  - [VaultBatcher](reference/sdk/VaultBatcher.md)
  - [web() transport](reference/sdk/RelayerWeb.md)
  - [node() transport](reference/sdk/RelayerNode.md)
  - [cleartext() transport](reference/sdk/RelayerCleartext.md)
  - [GenericProvider](reference/sdk/GenericProvider.md)
  - [GenericSigner](reference/sdk/GenericSigner.md)
  - [ViemProvider](reference/sdk/ViemProvider.md)
  - [ViemSigner](reference/sdk/ViemSigner.md)
  - [EthersProvider](reference/sdk/EthersProvider.md)
  - [EthersSigner](reference/sdk/EthersSigner.md)
  - [GenericStorage](reference/sdk/GenericStorage.md)
  - [Errors](reference/sdk/errors.md)
  - [Contract builders](reference/sdk/contract-builders.md)
  - [Event decoders](reference/sdk/event-decoders.md)
  - [Delegations](reference/sdk/delegation.md)
  - [Offline](reference/sdk/Offline.md)
  - [Network presets](reference/sdk/network-presets.md)
- [React reference](reference/react/README.md)
  - [ZamaProvider](reference/react/ZamaProvider.md)
  - [useConfidentialBalance](reference/react/useConfidentialBalance.md)
  - [useConfidentialBalances](reference/react/useConfidentialBalances.md)
  - [useConfidentialTransfer](reference/react/useConfidentialTransfer.md)
  - [useConfidentialTransferFrom](reference/react/useConfidentialTransferFrom.md)
  - [useConfidentialTransferFromAndCall](reference/react/useConfidentialTransferFromAndCall.md)
  - [useShield](reference/react/useShield.md)
  - [useApproveUnderlying](reference/react/useApproveUnderlying.md)
  - [useWrap](reference/react/useWrap.md)
  - [useUnshield](reference/react/useUnshield.md)
  - [useUnshieldAll](reference/react/useUnshieldAll.md)
  - [useResumeUnshield](reference/react/useResumeUnshield.md)
  - [usePendingUnshield](reference/react/usePendingUnshield.md)
  - [useUnwrap](reference/react/useUnwrap.md)
  - [useUnwrapAll](reference/react/useUnwrapAll.md)
  - [useFinalizeUnwrap](reference/react/useFinalizeUnwrap.md)
  - [useGrantPermit](reference/react/useGrantPermit.md)
  - [usePreparePermit](reference/react/usePreparePermit.md)
  - [useRegisterPermit](reference/react/useRegisterPermit.md)
  - [useHasPermit](reference/react/useHasPermit.md)
  - [useRevokePermits](reference/react/useRevokePermits.md)
  - [useClearCredentials](reference/react/useClearCredentials.md)
  - [useConfidentialSetOperator](reference/react/useConfidentialSetOperator.md)
  - [useConfidentialIsOperator](reference/react/useConfidentialIsOperator.md)
  - [useUnderlyingAllowance](reference/react/useUnderlyingAllowance.md)
  - [useWrapperDiscovery](reference/react/useWrapperDiscovery.md)
  - [useWrappersRegistryAddress](reference/react/useWrappersRegistryAddress.md)
  - [useListPairs](reference/react/useListPairs.md)
  - [useTokenPairsRegistry](reference/react/useTokenPairsRegistry.md)
  - [useTokenPairsLength](reference/react/useTokenPairsLength.md)
  - [useTokenPairsSlice](reference/react/useTokenPairsSlice.md)
  - [useTokenPair](reference/react/useTokenPair.md)
  - [useConfidentialTokenAddress](reference/react/useConfidentialTokenAddress.md)
  - [useTokenAddress](reference/react/useTokenAddress.md)
  - [useIsConfidentialTokenValid](reference/react/useIsConfidentialTokenValid.md)
  - [useMetadata](reference/react/useMetadata.md)
  - [useTotalSupply](reference/react/useTotalSupply.md)
  - [useZamaSDK](reference/react/useZamaSDK.md)
  - [useToken](reference/react/useToken.md)
  - [useWrappedToken](reference/react/useWrappedToken.md)
  - [useEncrypt](reference/react/useEncrypt.md)
  - [useDecryptValues](reference/react/useDecryptValues.md)
  - [useDecryptPublicValues](reference/react/useDecryptPublicValues.md)
  - [useDelegateDecryption](reference/react/useDelegateDecryption.md)
  - [useDelegatedDecryptValues](reference/react/useDelegatedDecryptValues.md)
  - [useRevokeDelegation](reference/react/useRevokeDelegation.md)
  - [useDelegationStatus](reference/react/useDelegationStatus.md)
  - [useDecryptBalanceAs](reference/react/useDecryptBalanceAs.md)
  - [useBatchDecryptBalancesAs](reference/react/useBatchDecryptBalancesAs.md)
  - [useVault](reference/react/useVault.md)
  - [useVaultBatcher](reference/react/useVaultBatcher.md)
  - [useDeposit](reference/react/useDeposit.md)
  - [useRedeem](reference/react/useRedeem.md)
  - [useJoin](reference/react/useJoin.md)
  - [useClaim](reference/react/useClaim.md)
  - [useQuit](reference/react/useQuit.md)
  - [useRecover](reference/react/useRecover.md)
  - [useDispatchBatch](reference/react/useDispatchBatch.md)
  - [useCurrentBatchId](reference/react/useCurrentBatchId.md)
  - [useBatchState](reference/react/useBatchState.md)
  - [useTimeUntilDispatchable](reference/react/useTimeUntilDispatchable.md)
  - [Query keys](reference/react/query-keys.md)

## Concepts

- [Architecture](concepts/architecture.md)
- [Permit model](concepts/permit-model.md)
- [Security model](concepts/security-model.md)
