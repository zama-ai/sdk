...
- **Chain preset export names have no `Config` suffix.** The chain presets exported from `@zama-fhe/sdk/chains` are named `sepolia`, `hoodi`, `mainnet`, `hardhat`, and `anvil`. Use `import { sepolia } from "@zama-fhe/sdk/chains"`, not `SepoliaConfig` or `HoodiConfig` (those are legacy relayer-sdk names that no longer apply).
