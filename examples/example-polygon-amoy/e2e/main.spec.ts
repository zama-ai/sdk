...
test.describe("main screen", () => {
  ...
  test("POL balance shows in header", async ({ page }) => {
    // POL row is always rendered; shows "—" until the query resolves.
  ...
  test("mint button is enabled once the registry loads a token", async ({ page }) => {
    // actionsDisabled = !isAmoy || !token. Once the first pair is auto-selected,
