...
describe("IndexedDBStorage", () => {
  ...
  describe("setItem / getItem", () => {
    it("stores and retrieves a string value", async () => {
      await storage.setItem("wallet-0x1", "encrypted-creds-json");

      const result = await storage.getItem("wallet-0x1");
    ...
    it("overwrites an existing value with put semantics", async () => {
      await storage.setItem("key", "value-1");
      await storage.setItem("key", "value-2");
    ...
    it("stores multiple keys independently", async () => {
      ...
      await storage.setItem("b", "bravo");

      expect(await storage.getItem("a")).toBe("alpha");
      expect(await storage.getItem("b")).toBe("bravo");
    });
  ...
  describe("clear", () => {
    it("removes all entries", async () => {
      await storage.setItem("a", "alpha");
      await storage.setItem("b", "bravo");
      await storage.setItem("c", "charlie");
    ...
    it("allows storing new items after clear", async () => {
      ...
      expect(await storage.getItem("key")).toBe("after");
    });
  });
  ...
  describe("lazy initialization via #getDB()", () => {
    ...
    it("reuses the same database connection across operations", async () => {
      ...
      const a = await storage.getItem("a");
      const b = await storage.getItem("b");
  ...
  describe("version change and close events", () => {
    it("recovers after the database is closed externally", async () => {
      ...
      await freshStorage.setItem("key", "initial");
      ...
      const result = await newStorage.getItem("key");
  ...
  describe("custom DB name and version", () => {
    it("uses the provided database name", async () => {
      ...
      await custom.setItem("key", "value");
      expect(await custom.getItem("key")).toBe("value");
    });

    it("uses the default name and version when none provided", async () => {
      const defaultStorage = new IndexedDBStorage();
      await defaultStorage.setItem("test", "value");
      expect(await defaultStorage.getItem("test")).toBe("value");
    });
  });
});
