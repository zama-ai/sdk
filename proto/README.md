...
`EventDelivery` includes the context ID, RPC operation ID and a monotonically increasing sequence number. Lifecycle payloads separately preserve the SDK's optional `operationId` as `sdk_operation_id`; RPC correlation never replaces it. SDK work outside a unary operation has an empty RPC operation ID. Work that continues after unary cancellation retains its RPC operation ID; correlation does not mean the RPC is still active. Sequence numbers continue across explicit reattachment, but they do not provide replay or durable resumption. After attachment, clients ignore unknown server frame types. For an unknown delivery payload, they still validate its context and sequence, then acknowledge and skip it so the delivery window stays open. Rust's protobuf decoder discards unknown fields, so an empty frame or payload follows the same skip behavior; Go rejects empty frames without unknown wire fields.
...
The server permits at most 256 unacknowledged event deliveries; attempting another terminates the subscription with `EVENT_BACKPRESSURE`. Separately, signer, storage and event writers each allow 256 queued output frames while honoring HTTP/2 backpressure. Exceeding that output bound terminates the affected channel with `CALLBACK_BACKPRESSURE`; there is no silent event dropping or unbounded buffering. Native clients maintain bounded queues and fail on malformed correlation or queue overflow. Delivery order describes this subscription's emitted order, not a global ordering across SDK contexts.

Go `EventSubscription.Close` cancels queued and active callbacks; `SDKContext.Close` also closes the subscription. Go `SDKContext.StopEvents` also stops a subscription installed through `SDKConfig.Events`. Rust closes the channel and drops callback futures when the managed SDK closes or drops. Observe termination through Go `WaitChannelFailure(ctx, EventChannel)` or Rust `wait_channel_closed(CallbackChannel::Events)`. Go supports explicit reattachment; replacing a closed or failed subscription retries `EVENT_ATTACHED` for up to roughly one second while the server observes the old stream closing. An active subscription retains exclusive ownership; retries do not displace it. The caller's cancellation still applies. Rust applications build a new managed SDK. Neither client automatically replays operations or callbacks. Events emitted before attachment or while disconnected are not retained.

An event-channel failure clears pending notifications but does not cancel independent SDK work. Notification callbacks that ignore cancellation can delay their own cleanup in Go. Go recovers a handler panic as `CALLBACK_FAILED`; a Rust handler panic terminates the subscription. Avoid awaiting an operation that needs the same serialized notification handler from inside that handler.

Event transport errors include `EVENT_ATTACHED` for a duplicate attachment, `EVENT_ATTACH_TIMEOUT` when no context is attached in time, `EVENT_BACKPRESSURE` when the unacknowledged delivery bound is exceeded, `CALLBACK_BACKPRESSURE` when the output queue is full, `EVENT_ENCODING_FAILED` when an SDK notification cannot be encoded, and `EVENT_DELIVERY_NOT_FOUND` for a stale reply. Encoding failures terminate the channel with a safe message so listeners can detect lost delivery.
