...
describe("handle", () => {
  ...
  it("computeInputHandle validates index range", () => {
    ...
    expect(() =>
      computeInputHandle(mockCiphertext, -1, FheType.Uint8, FHEVM_ADDRESSES.acl, 31_337n),
    ).toThrow(/between 0 and 255/i);
    expect(() =>
      computeInputHandle(mockCiphertext, 256, FheType.Uint8, FHEVM_ADDRESSES.acl, 31_337n),
    ).toThrow(/between 0 and 255/i);
    expect(() =>
      computeInputHandle(mockCiphertext, 1.5, FheType.Uint8, FHEVM_ADDRESSES.acl, 31_337n),
    ).toThrow(/between 0 and 255/i);
  });
});
