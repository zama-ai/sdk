...
describe("eip712", () => {
  it("INPUT_VERIFICATION_EIP712 typehash matches spec definition", () => {
    ...
    const encoded = ethers.TypedDataEncoder.from(inputTypes).encodeType("CiphertextVerification");
  ...
  it("KMS_DECRYPTION_EIP712 typehash matches on-chain definition", () => {
    ...
    const encoded = ethers.TypedDataEncoder.from(kmsTypes).encodeType("PublicDecryptVerification");
