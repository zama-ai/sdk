...
// All tests in this suite start from the connect screen and click Connect Wallet.
// WagmiSigner does not auto-connect without stored localStorage state, so each test
// goes through the explicit connect flow before reaching the main screen.
...
test.describe("main screen", () => {
  ...
  test("renders all operation cards", async ({ page }) => {
    await expect(page.getByText("Clear Signing Intent Console")).toBeVisible();
    await expect(page.getByText("Balances")).toBeVisible();
  ...
  test("shows clear-signing preview actions", async ({ page }) => {
    await expect(page.getByRole("button", { name: "Preview intent" })).toHaveCount(5);
  });

  test("renders a human-readable clear-signing preview", async ({ page }) => {
    const balancesCard = page.locator(".card", { hasText: "Balances" });
